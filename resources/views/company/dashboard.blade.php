...
<div class="space-y-6">
    ...
    <div class="md:flex md:items-center md:justify-between">
        ...
        <div class="mt-4 flex md:ml-4 md:mt-0 space-x-3">
            ...
            <button type="button" onclick="window.location.href='/company/{{ request()->route('tenant') }}/inventory/items/create'" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="relative overflow-hidden rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 px-4 py-5 shadow sm:px-6">
            ...
            <dd class="ml-16 flex items-baseline">
                <p class="text-2xl font-semibold text-white">{{ $stats['total_items'] ?? 1245 }}</p>
        ...
        <div class="relative overflow-hidden rounded-lg bg-gradient-to-br from-red-500 to-red-600 px-4 py-5 shadow sm:px-6">
            ...
            <dd class="ml-16 flex items-baseline">
                <p class="text-2xl font-semibold text-white">{{ $stats['low_stock'] ?? 23 }}</p>
                <p class="ml-2 flex items-baseline text-sm font-semibold text-white/90">
                    Needs attention
                </p>
            </dd>
        ...
        <div class="relative overflow-hidden rounded-lg bg-gradient-to-br from-green-500 to-green-600 px-4 py-5 shadow sm:px-6">
            ...
            <dd class="ml-16 flex items-baseline">
                <p class="text-2xl font-semibold text-white">${{ number_format($stats['value'] ?? 125600) }}</p>
                <p class="ml-2 flex items-baseline text-sm font-semibold text-white/90">
                    <svg class="h-5 w-5 flex-shrink-0 self-center" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M10 17a.75.75 0 01-.75-.75V5.612L5.29 9.77a.75.75 0 01-1.08-1.04l5.25-5.5a.75.75 0 011.08 0l5.25 5.5a.75.75 0 11-1.08 1.04l-3.96-4.158V16.25A.75.75 0 0110 17z" clip-rule="evenodd" />
                    </svg>
        ...
        <div class="relative overflow-hidden rounded-lg bg-gradient-to-br from-purple-500 to-purple-600 px-4 py-5 shadow sm:px-6">
            ...
            <dd class="ml-16 flex items-baseline">
                <p class="text-2xl font-semibold text-white">{{ $warehouses ?? 5 }}</p>
                <p class="ml-2 flex items-baseline text-sm font-semibold text-white/90">
                    Locations
                </p>
            </dd>
...
<div class="rounded-lg bg-white shadow">
    ...
    <div class="divide-y divide-gray-200">
        ...
        <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50">
            <div class="flex items-center">
                <div class="h-10 w-10 rounded-full {{ $movement->type === 'in' ? 'bg-green-100' : 'bg-red-100' }} flex items-center justify-center">
                    <svg class="h-5 w-5 {{ $movement->type === 'in' ? 'text-green-600' : 'text-red-600' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        @if($movement->type === 'in')
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-900">{{ $movement->item->name ?? 'Unknown Item' }}</p>
        ...
        <div class="rounded-lg bg-white shadow">
            <div class="px-6 py-5 border-b border-gray-200 flex items-center justify-between">
                ...
                <a href="/company/{{ request()->route('tenant') }}/inventory/items?filter=low_stock" class="text-sm text-indigo-600 hover:text-indigo-500">View all</a>
            </div>
            <div class="divide-y divide-gray-200">
                @forelse($lowStockAlerts as $item)
                <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50">
                    <div class="flex items-center flex-1">
                        <img class="h-10 w-10 rounded object-cover" src="{{ $item->image ?? 'https://via.placeholder.com/40' }}" alt="">
                    ...
                    <div class="text-right">
                        <p class="text-sm font-semibold text-red-600">{{ $item->quantity }} left</p>
                        <p class="text-xs text-gray-500">Min: {{ $item->min_quantity }}</p>
                    </div>
                </div>
                @empty
                <div class="px-6 py-8 text-center text-sm text-gray-500">
