...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <a href="/company/{{ request()->route('tenant') }}/inventory/items/create" 
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-5 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-2">{{ $stats['total_items'] ?? 1245 }}</p>
                </div>
        ...
        <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl p-5 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-2">{{ $stats['low_stock'] ?? 23 }}</p>
                </div>
                <div class="bg-white/20 p-3 rounded-lg">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-5 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-2">${{ number_format($stats['value'] ?? 125600) }}</p>
                </div>
                <div class="bg-white/20 p-3 rounded-lg">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-5 text-white shadow-lg">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-2">{{ $warehouses ?? 5 }}</p>
                </div>
                <div class="bg-white/20 p-3 rounded-lg">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <div class="flex items-center space-x-4">
                        <div class="h-10 w-10 rounded-full {{ $movement->type === 'in' ? 'bg-green-100 dark:bg-green-900/30' : 'bg-red-100 dark:bg-red-900/30' }} flex items-center justify-center">
                            <svg class="h-5 w-5 {{ $movement->type === 'in' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                @if($movement->type === 'in')
                ...
                <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <div class="flex items-center space-x-4">
                        <img class="h-10 w-10 rounded-lg object-cover" 
                             src="{{ $item->image ?? 'https://images.unsplash.com/photo-1580910654019-3bd4c8e4e8b4?ixlib=rb-1.2.1&auto=format&fit=crop&w=100&q=80' }}" 
                             alt="{{ $item->name }}">
                        <div>
                            <p class="text-sm font-medium text-gray-900 dark:text-white truncate max-w-[150px]">{{ $item->name }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">SKU: {{ $item->sku }}</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <p class="text-sm font-semibold text-red-600 dark:text-red-400">{{ $item->quantity }} left</p>
