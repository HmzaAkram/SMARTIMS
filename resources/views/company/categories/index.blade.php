...
@section('title', 'Categories - SmartIMS')
...
<div class="space-y-6">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        ...
        <div class="mt-4 sm:mt-0">
            <a href="{{ route('company.categories.create', ['tenant' => request()->route('tenant')]) }}" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
    ...
    <div class="rounded-lg bg-white shadow p-4">
        ...
        <div class="mt-4 flex items-center justify-between">
            ...
            <div class="text-sm text-gray-500">
                Showing {{ $categories->firstItem() ?? 1 }} to {{ $categories->lastItem() ?? 10 }} of {{ $categories->total() ?? 0 }} results
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ $category->description ?? 'No description' }}</td>
                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-900">{{ $category->items_count }}</td>
                        <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <div class="flex justify-end space-x-2" x-data="{ open: false }">
                                <a href="{{ route('company.categories.edit', [request()->route('tenant'), $category]) }}" class="text-gray-600 hover:text-gray-900">Edit</a>
                                    ...
                                    <div class="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                                        ...
                                        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                                            ...
                                            <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                                                <form method="POST" action="{{ route('company.categories.destroy', [request()->route('tenant'), $category]) }}" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-14 text-center">
                            ...
                            <div class="mt-6">
                                <a href="{{ route('company.categories.create', request()->route('tenant')) }}" class="inline-flex items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
                                    <svg class="-ml-0.5 mr-1.5 h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M10.75 4.75a.75.75 0 00-1.5 0v4.5h-4.5a.75.75 0 000 1.5h4.5v4.5a.75.75 0 001.5 0v-4.5h4.5a.75.75 0 000-1.5h-4.5v-4.5z" />
                                    </svg>
                                    Add Category
