...
@section('title', 'Create Sales Order - SMARTIMS')
...
<div class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg">
        <form action="{{ route('company.sales.store', $tenant) }}" method="POST" id="salesOrderForm">
            @csrf
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <input type="date" name="order_date" id="order_date" value="{{ old('order_date', date('Y-m-d')) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        @error('order_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                                ...
                                <div class="col-span-5">
                                    <select name="items[0][item_id]" class="item-select block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                        ...
                                        @foreach($items as $item)
                                            <option value="{{ $item->id }}" data-price="{{ $item->selling_price ?? $item->unit_price }}" data-stock="{{ $item->inventory->quantity ?? 0 }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            <div>
                                ...
                                <textarea name="shipping_address" id="shipping_address" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('shipping_address') }}</textarea>
                            </div>
                            
                            <div>
                                <label for="billing_address" class="block text-sm font-medium text-gray-700">Billing Address</label>
                                <textarea name="billing_address" id="billing_address" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('billing_address') }}</textarea>
                            </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="grid grid-cols-3 gap-2">
                                        <div>
                                            ...
                                            <input type="number" name="discount" id="discount" step="0.01" min="0" value="{{ old('discount', 0) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <label for="tax" class="block text-xs text-gray-600">Tax</label>
                                            <input type="number" name="tax" id="tax" step="0.01" min="0" value="{{ old('tax', 0) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <label for="shipping_cost" class="block text-xs text-gray-600">Shipping</label>
                                            <input type="number" name="shipping_cost" id="shipping_cost" step="0.01" min="0" value="{{ old('shipping_cost', 0) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                    </div>
                ...
                <div class="border-t border-gray-200 pt-6">
                    <div class="flex justify-end space-x-3">
                        <a href="{{ route('company.sales.index', $tenant) }}" class="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-900 shadow-sm hover:bg-gray-300">
                            Cancel
                        </a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function loadItems() {
            ...
            fetch(`/company/{{ $tenant }}/sales/items?warehouse_id=${warehouseId}`, {
                headers: {
