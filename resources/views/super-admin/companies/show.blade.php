...
@section('title', $company->name . ' - Company Details - SmartIMS')
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between">
            <div class="flex items-center space-x-4">
                <div class="w-16 h-16 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center">
                    <span class="text-white text-2xl font-bold">{{ strtoupper(substr($company->name, 0, 2)) }}</span>
                </div>
                <div>
                    ...
                    <div class="flex items-center space-x-4 mt-2">
                        ...
                        <span class="text-gray-600">
                            <i class="fas fa-globe mr-1"></i> {{ $company->domain }}.smartims.test
                        </span>
                        <span class="text-gray-600">
                            <i class="fas fa-database mr-1"></i> {{ $company->database }}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-1">{{ $company->users->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg font-bold mt-1">{{ $company->subscription->plan_name ?? 'N/A' }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold mt-1">${{ $company->subscription->price ?? 0 }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg font-bold mt-1">{{ $company->created_at->format('M d, Y') }}</p>
                </div>
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-gray-900">{{ $company->name }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Email</label>
                            <p class="mt-1 text-gray-900">{{ $company->email }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Phone</label>
                            <p class="mt-1 text-gray-900">{{ $company->phone ?? 'Not provided' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Domain</label>
                            <p class="mt-1 text-gray-900">{{ $company->domain }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Database</label>
                            <p class="mt-1 text-gray-900">{{ $company->database }}</p>
                        </div>
                        <div>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-gray-900">{{ $company->subscription->plan_name }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Price</label>
                            <p class="mt-1 text-gray-900">${{ $company->subscription->price }}/month</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Billing Cycle</label>
                            <p class="mt-1 text-gray-900">{{ ucfirst($company->subscription->billing_cycle) }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Status</label>
                            <p class="mt-1">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-gray-900">{{ $company->subscription->trial_ends_at->format('M d, Y') }}</p>
                        </div>
                        @endif
                        @if($company->subscription->ends_at)
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Subscription Ends</label>
                            <p class="mt-1 text-gray-900">{{ $company->subscription->ends_at->format('M d, Y') }}</p>
                        </div>
                        @endif
                    </div>
                    
                    <!-- Plan Features -->
                    @if($company->subscription->features)
                    <div class="mt-6">
                        <label class="block text-sm font-medium text-gray-500 mb-2">Plan Features</label>
                        <div class="grid grid-cols-2 gap-3">
                            @foreach($company->subscription->features as $feature => $value)
                            <div class="flex items-center">
                                <i class="fas fa-check text-green-500 mr-2"></i>
                                <span class="text-sm text-gray-700">{{ ucfirst($feature) }}: {{ $value }}</span>
                            </div>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('admin.payments.index') }}?search={{ $company->name }}" 
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-sm text-gray-500">{{ $company->users->count() }} users</span>
                </div>
                <div class="space-y-3">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="space-y-3">
                    @if($company->subscription && $company->subscription->status === 'trialing')
                    <form action="{{ route('admin.companies.reset-trial', $company->id) }}" method="POST">
                        @csrf
